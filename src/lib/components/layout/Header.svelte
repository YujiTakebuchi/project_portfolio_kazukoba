<script lang="ts">
	 ...
	 * PC / SP とも position: sticky で画面上部に貼り付く。
	 ...
	 * <dialog> はトップレイヤーに出るのでページ本来のヘッダーは背面に
	 * 隠れてしまう。そのためロゴはパネル側にも組んでいる（WORKS の
	 * 拡大表示がフッターを持っているのと同じ考え方）。ヘッダーが
	 * 貼り付いている位置とぴったり重なるように寸法を揃えてあるので、
	 * 開閉してもロゴは動かず、ハンバーガーはその場でバツ印に変わる。
	 *
...
<style lang="scss">
	...
	.header {
		// PC / SP とも画面上部に貼り付く。
		//
		// position: fixed と違って通常フローに残るので、下のコンテンツを
		// margin で押し下げる必要がなく、各ページの余白はそのまま使える。
		// スクロールを担当するのは body で、.split の overflow-x: clip は
		// スクロールコンテナを作らない（clip は hidden と違う）ため、
		// ページ全体を通して効く。
		position: sticky;
		top: 0;
		// 本文より前、ドロワー（100）やモーダル（200）より後ろ
		z-index: 10;
		...
		// --- 下をくぐるコンテンツを隠すための地 ---
		//
		// ボックスにそのまま background を敷くと 3 つ足りない。
		//   - 幅がベース幅どまりなので、m.fullBleed() で画面幅まで広げた
		//     KV や WORKS のスライダーが左右の .side を通って見えてしまう
		//   - PC ナビの現在地の下線がボックスの下端より少し下にあり、
		//     本文の上に線だけ浮いて残ってしまう
		//   - カンプのヘッダーは文字の下端がそのまま下端（SP 50 / PC 56）で、
		//     スクロールした本文が文字のすぐ際で切れて窮屈に見える
		// どれもレイアウトを動かさずに済ませたいので、背景だけを
		// 疑似要素で画面幅ぶん・下に伸ばして塗る。
		//
		// 下の伸ばし幅は上のパディングのおよそ半分。天地中央にはせず、
		// 文字の下に余白があるように見せるための値。
		&::before {
			content: "";
			position: absolute;
			// ヘッダーの文字より後ろ、本文より前
			...
			top: 0;
			bottom: f.vw(-14);
			// m.fullBleed() と同じ計算。親の幅と画面幅の差を左右に振る
			left: calc((100% - var(--screen-w)) / 2);
			right: calc((100% - var(--screen-w)) / 2);
